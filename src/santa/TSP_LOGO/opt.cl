/*
 *   Logo TSP Solver ver. 0.5  Copyright (C) 2012  Kamil Rocki
 *
...
int inline calculateDistance2DSimple(unsigned int i, unsigned int j, __local city_coords* localCoords) {
...
__kernel void cl2optKernel(__global const city_coords* dCoords,  __global struct best2_out* dResult, __local city_coords* localCoords, __local best2_out* best_values, int BLOCK_SIZE, unsigned int cities, int number_of_2opts, int iterations) {
    ...
    for (int no = 0; no < iter; no++) {
        ...
        if (id < max) {
            ...
            change = //calculate_change(i, j, localCoords);
                ...
                calculateDistance2DSimple(i, j + 1,   localCoords) +  calculateDistance2DSimple(i - 1, j, localCoords)
                - calculateDistance2DSimple(i, i - 1, localCoords)   -  calculateDistance2DSimple(j + 1, j, localCoords);
